...
import '../../data/mappers/movie_mappers/get_popular_movies_response_mapper.dart';
import '../../domain/usecases/get_top_popular_movies_usecase.dart';
...
import '../../domain/repositories_interfaces/movie_repository_interface.dart';
...
extension GetItInjectableX on GetIt {
  GetIt init({
    ...
    gh.factory<GetPopularMoviesResponseMapper>(
        () => GetPopularMoviesResponseMapper(get<Configuration>()));
    gh.factory<GetTrendingMoviesResponseMapper>(
        () => GetTrendingMoviesResponseMapper(get<Configuration>()));
    gh.factory<GetTopPopularMoviesUsecase>(
        () => GetTopPopularMovies(get<IMovieRepository>()));
    ...
    gh.factory<HomeController>(() => HomeController(
          ...
          get<GetTopPopularMoviesUsecase>(),
        ));

    // Eager singletons must be registered in the right order
    gh.singleton<IHttpClient>(HttpClient(get<Configuration>()));
    gh.singleton<IMovieRepository>(MovieRepository(
      get<IHttpClient>(),
      get<Configuration>(),
      get<GetPopularMoviesResponseMapper>(),
    ));
